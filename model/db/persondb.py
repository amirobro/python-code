...
cursor = db.cursor()
cursor.execute(
"insert into person_tbl(NAME,FAMILY,NATIONAL_CODE) values(%s,%s,%s)",
...
cursor = db.cursor()
cursor.execute(
'update person_tbl set NAME=%s,FAMILY=%s,NATIONAL_CODE=%s where ID=%s',
...
cursor = db.cursor()
cursor.execute(
"select * from person_tbl where FAMILY=%s",
[family]
)
